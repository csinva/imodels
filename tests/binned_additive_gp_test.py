"""Tests for AddGPRegressor, the binned additive Gaussian-process GAM."""
...
from imodels.algebraic.binned_additive_gp import AddGPRegressor as _AddGP
...
class TestAddGPRegressor:
    def test_fits_an_additive_signal(self):
        X, y = _additive_data()
        Xtr, Xte, ytr, yte = train_test_split(X, y, random_state=0)
        model = _AddGP(**FAST).fit(Xtr, ytr)
    ...
    def test_recovers_a_pairwise_interaction(self):
        ...
        model = _AddGP(schedule=False, n_bins=10, n_pairs=1, pair_bins=6,
                       n_steps=25).fit(X, y)
    ...
    def test_prediction_is_deterministic(self):
        ...
        a = _AddGP(**FAST).fit(X, y).predict(X)
        b = _AddGP(**FAST).fit(X, y).predict(X)
        np.testing.assert_allclose(a, b)

    def test_shape_function_matches_the_model(self):
        X, y = _additive_data(n=300, seed=2)
        model = _AddGP(**FAST).fit(X, y)
    ...
    def test_exposed_in_the_package_namespace(self):
        assert AddGPRegressor is _AddGP
