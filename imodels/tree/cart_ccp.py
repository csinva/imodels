...
import sys
sys.path.append(".")
from shrunk_tree import ShrunkTreeRegressor,ShrunkTreeRegressorCV
...
if __name__ == '__main__':
    m = DecisionTreeRegressorCCP(estimator_=DecisionTreeRegressor(random_state = 1),desired_complexity = 10)
    X_train,y_train = make_friedman1()
    X_test,y_test = make_friedman1()
    #X, y = datasets.load_breast_cancer(return_X_y=True)
    m.fit(X_train,y_train)
    m.predict(X_test)
    print(m.score(X_test,y_test))
    
    m = ShrunkDecisionTreeRegressorCCP_CV(estimator_=DecisionTreeRegressor(random_state=1),desired_complexity = 10,reg_param_list = [0.0])
    m.fit(X_train,y_train)
    print(m.score(X_test,y_test))
