    ...
    def fit(
        ...
        while len(potential_splits) > 0 and not finished:
            ...
            for tree_num_ in list(range(len(self.trees_))) + [-1]:
                ...
                # subtract predictions of all other trees. Since the current tree makes a constant prediction over the
                # node being split, one may ignore its contributions to the residuals without affecting the
                # impurity decrease.
