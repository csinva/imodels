...
class GreedyRuleListClassifier(BaseEstimator, RuleList, ClassifierMixin):
    def __init__(self, max_depth: int = 5, class_weight=None, criterion: str = "gini"):
        """
        Params
        ------
        max_depth
    ...
    def fit_node_recursive(self, X, y, depth: int, verbose):
        ...
        # recursively generate rule list
            ...
            par_node = [
                {
                    ...
                    "val": np.mean(
                        y_left
                    ),  # will be the values before splitting in the next lower level
                    "flip": flip,
            ...
            par_node = par_node + self.fit_node_recursive(
                x_left, y_left, depth + 1, verbose=verbose
            )
